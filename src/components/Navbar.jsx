    ...
    return (
        <>
            ...
            <nav className={`${activemobile ? 'block' : 'hidden'} fixed bg-black/90 md:bg-black h-full w-full md:w-[15rem] z-30 md:block`}>
                ...
                >
                    <Link to="/" className="flex flex-col justify-center items-center m-7 gap-2" onClick={() => setActivemobile(!activemobile)}>
                        <h1 className="text-yellow-300/70 font-bold text-2xl text-center">SpotFlix</h1>
                    </Link>
                </motion.div>
                ...
                <ul className="text-white font-semibold text-[16px] text-center px-5">
                    {Navdata.map((data) => (
                            <Link key={data.id} to={data.link}><li className={`${header == data.headername ? 'bg-yellow-500/20 border-yellow-500 text-white' : 'bg-gray-500/20 border-black'} p-2 my-2  hover:bg-yellow-500/20 rounded-[5px] border-2 hover:border-yellow-600`} onClick={() => setActivemobile(!activemobile)}>{data.Name}</li></Link>
                    ))}
                ...
                <div className="absolute bottom-0 w-full p-5 md:p-2 text-white">
                    {user ? <>
                        <div className="w-full bg-gray-900 px-5 py-2 gap-4 rounded-xl flex items-center font-semibold border-2 border-blue-100/10">
                            <img src={user.photoURL == null ? User : user.photoURL} alt="user" className="h-10 rounded-full" /> //for the user picture 
                            <h1>{user.displayName}</h1>
                        </div>

                        <div className="cursor-pointer bg-red-500 flex justify-center items-center p-2 rounded-xl mt-2" onClick={()=>Any}>
                            <h1>Logout</h1>
                        </div>
                    </>
                        :
