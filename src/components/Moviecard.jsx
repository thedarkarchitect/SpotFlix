    ...
    return (
        <motion.div
            ...
            <div className='absolute bottom-0 w-full flex justify-between items-end p-3 z-20'>
                ...
                {(movie.vote_average||0) > 7 ? <h1 className='font-bold text-lime-400 p-2 bg-zinc-900 rounded-full'>{(movie.vote_average||0).toFixed(1)}</h1> : (movie.vote_average||0) > 5.5 ? <h1 className='font-bold text-fuchsia-400 p-2 bg-zinc-900 rounded-full'>{(movie.vote_average||0).toFixed(1)}</h1> : <h1 className='font-bold text-red-600 p-2 bg-zinc-900 rounded-full'>{(movie.vote_average||0).toFixed(1)}</h1>}
            </div>
